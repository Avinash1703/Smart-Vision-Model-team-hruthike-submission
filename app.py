    ...
    if page == "Signup/Login":
        ...
        if st.session_state.get("logged_in"):
            ...
            if image is not None:
                ...
                st.markdown('<div class="subtitle">Prediction Results</div>', unsafe_allow_html=True)
                ...
                with st.container():
                    ...
                    if product_type == "Fruit/Vegetable":
                        ...
                        st.markdown(f"<div class='result-box'><strong>Predicted Fruit/Vegetable Class:</strong> <span class='highlight'>{fruit_name}</span></div>", unsafe_allow_html=True)
                        st.markdown(f"<div class='result-box'><strong>Predicted Freshness:</strong> <span class='highlight'>{freshness}</span></div>", unsafe_allow_html=True)

                        if freshness_label == 0:
                            shelf_life_info = fruit_vegetable_shelf_life(image)
                            st.markdown(f"<div class='result-box'><strong>Estimated Shelf Life:</strong> {shelf_life_info}</div>", unsafe_allow_html=True)

                    elif product_type == "Bread":
                        freshness_info = bread_freshness(image)
                        st.markdown(f"<div class='result-box'><strong>Bread Freshness and Shelf Life Info:</strong> {freshness_info}</div>", unsafe_allow_html=True)
        ...
        if st.session_state.get("logged_in"):
            ...
            if image is not None:
                items_info = count_cart_items(image)

                # Show detected items information in a styled box
                st.markdown(f"""
                    <div class='result-box'>
                        <strong>Detected Items Information:</strong><br>
                        <span class='highlight-result'>{items_info}</span>
                    </div>
                """, unsafe_allow_html=True)
        ...
        if st.session_state.get("logged_in"):
            ...
            if image is not None:
                items_info = extract_product_info(image)

                # Show extracted information in a styled box
                st.markdown(f"""
                    <div class='result-box'>
                        <strong>Extracted Information:</strong><br>
                        <span class='highlight-result'>{items_info}</span>
                    </div>
                """, unsafe_allow_html=True)
